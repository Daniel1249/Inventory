...
namespace Activity1101_LayeringOurSolution
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            static void DeleteMultipleItems()
            {
                Console.WriteLine("Would you like to delete items as a batch?");
                bool batchDelete = Console.ReadLine().StartsWith("y", StringComparison.
                OrdinalIgnoreCase);
                ...
                while (deleteAnother == true)
                {
                    ...
                    deleteAnother = Console.ReadLine().StartsWith("y",
                    StringComparison.OrdinalIgnoreCase);
                    if (batchDelete && !deleteAnother)
                    {
                        ...
                        allItems.ForEach(x => Console.Write($"{x},"));
                        Console.WriteLine();
                        if (Console.ReadLine().StartsWith("y", StringComparison.
                        OrdinalIgnoreCase))
                        {
